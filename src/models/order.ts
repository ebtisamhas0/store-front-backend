...
import { order1, ProductToOrder } from '../interfaces/orders_interface'

export class OrderStore {
    async getOrders(): Promise<order1[]> {
        try {
            // @ts-ignore
    ...
    async getOrderById(id: number): Promise<order1> {
        try {
    ...
    async createOrder(o: order1): Promise<order1> {
        try {
            const sql =
    ...
    async updateOrder(o: order1): Promise<order1> {
        try {
    ...
    async deleteOrder(id: number): Promise<order1> {
        try {
            // @ts-ignore
